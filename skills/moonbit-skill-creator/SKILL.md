---
name: moonbit-skill-creator
description: Guide for creating effective skills, with MoonBit-specific guidance for skills that use MoonBit packages or tooling. Use when users want to create or update a skill, especially when the skill should include MoonBit templates, scripts, or project scaffolding.
license: Apache 2.0
---

# MoonBit Skill Creator

This skill provides guidance for creating effective skills, with extra guidance
for MoonBit-based skills.

## About Skills

Skills are modular, self-contained packages that extend Agent's capabilities by
providing specialized knowledge, workflows, and tools. Think of them as
"onboarding guides" for specific domains or tasks - they transform Agent from a
...
everything else Agent needs: system prompt, conversation history, other Skills'
metadata, and the actual user request.

**Default assumption: Agent is already very smart.** Only add context Agent
doesn't already have. Challenge each piece of information: "Does Agent really
...
Think of Agent as exploring a path: a narrow bridge with cliffs needs specific
...
- **Frontmatter** (YAML): Contains `name` and `description` fields. These are
  the only fields that Agent reads to determine when the skill gets used, thus
...
- **Benefits**: Token efficient, deterministic, may be executed without loading
  into context
- **Note**: Scripts may still need to be read by Agent for patching or
  environment-specific adjustments

**MoonBit template recommendation:** When a skill uses MoonBit packages or
tooling, include a full MoonBit template under `scripts/` with `moon.pkg.json`,
`moon.mod.json`, `.gitignore`, and `main.mbt` as the minimum starting point.
This keeps MoonBit-based skills portable and reproducible.

##### References (`references/`)

Documentation and reference material intended to be loaded as needed into
context to inform Agent's process and thinking.

- **When to include**: For documentation that Agent should reference while
  working
...
- **Benefits**: Keeps SKILL.md lean, loaded only when Agent determines it's
  needed
...
- **Benefits**: Separates output resources from documentation, enables Agent to
...
3. **Bundled resources** - As needed by Agent (Unlimited because scripts can be
...
- **Structure longer reference files** - For files longer than 100 lines,
  include a table of contents at the top so Agent can see the full scope when
  previewing.
...
**MoonBit note:** When the skill will build or run MoonBit packages, plan a
reusable MoonBit template under `assets/` so future tasks can start from a
known-good baseline.
...
being created for another instance of Agent to use. Include information that
would be beneficial and non-obvious to Agent. Consider what procedural
...
- **MoonBit skills**: See references/moonbit.md for developing MoonBit related
  skills
...
**MoonBit template recommendation (required for MoonBit skills):** When creating
skills using MoonBit packages, include a full template under `scripts/` with
`moon.pkg.json`, `moon.mod.json`, `.gitignore`, and `main.mbt` as the minimum
start. Add any extra MoonBit files or configs only if the skill needs them.
...
- `description`: This is the primary triggering mechanism for your skill, and
  helps Agent understand when to use the skill.
  ...
  - Example description for a `docx` skill: "Comprehensive document creation,
    ...
    preservation, and text extraction. Use when Agent needs to work with
